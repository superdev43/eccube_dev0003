...
class ShoppingController extends AbstractShoppingController
{
    ...
    public function index(PurchaseFlow $cartPurchaseFlow)
    {
        ...
        foreach ($OrderProductItems as $OrderProductItem) {
            if($OrderProductItem->getProduct()->no_fee != 1){
                ...
                if($OrderProductItem->getProduct()->shipping_charge == Null){
                    ...
                    $OrderProductItem->setSynDeliveryFeeTotal($delivery_fee);
                    ...
                    $em->persist($OrderProductItem);
                    $em->flush();
                    $orderShippingChargeSum += $OrderProductItem->getProduct()->shipping_charge * $OrderProductItem->getQuantity();
                }
            }else{
                $OrderProductItem->setSynDeliveryFeeTotal(0);
                $em = $this->getDoctrine()->getManager();
                $em->persist($OrderProductItem);
                $em->flush();
            }
        ...
        return [
            ...
            'Order' => $Order,
            'orderShippingChargeSum' => $orderShippingChargeSum
        ];
    }
