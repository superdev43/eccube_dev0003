...
namespace Plugin\CustomShipping\Service;
...
class OrderPdfService extends TcpdfFpdi
{
     ...
     */
    public function __construct(EccubeConfig $eccubeConfig, OrderRepository $orderRepository, OrderItemRepository $orderItemRepository, ShippingRepository $shippingRepository, TaxRuleService $taxRuleService, BaseInfoRepository $baseInfoRepository, EccubeExtension $eccubeExtension, TaxExtension $taxExtension, CusShippingRepository $cusShippingRepository, CusShipping $cusShipping)
    {
    ...
    public function makePdf(array $formData)
    {
        ...
        foreach ($ids as $id) {
            ...
            $CusShipping = $this->cusShippingRepository->find($id);
            if (!$CusShipping) {
                // 出荷情報の取得ができなかった場合
                continue;
            }
            $OrderItems = $this->orderItemRepository->findBy(['cus_shipping_id'=>$id]);
            // テンプレートファイルを読み込む
            ...
            // 注文情報を描画する
            $this->renderOrderData($Order, $id);

            // メッセージを描画する
            $this->renderMessageData($formData);

            // 出荷詳細情報を描画する
            $this->renderOrderDetailData($OrderItems,$id);
     ...
     * 
     */
    protected function renderOrderData($Order, $cusShippingId)
    {
        ...
        if (!$Order->isMultiple()) {
            ...
            $paymentTotalText = $this->eccubeExtension->getPriceFilter($cusShipping->getCusPaymentTotal());
     ...
     */
    protected function renderOrderDetailData($OrderItems, $cusShippingId)
    {
        $arrOrder = [];
        ...
        $OrderList = $this->orderRepository->findBy(['id'=>$OrderItems[0]->getOrderId()]);
        $Order = $OrderList[0];
        ...
        if (!$Order->isMultiple()) {
            ...
            $arrOrder[$i][1] = '';
            $arrOrder[$i][2] = '商品合計';
            $arrOrder[$i][3] =  $cusShipping->getCusSubTotal();

            ++$i;
            $arrOrder[$i][0] = '';
            $arrOrder[$i][1] = '';
            $arrOrder[$i][2] = '送料';
            $arrOrder[$i][3] = $cusShipping->getCusShippingCharge();
            ...
            $arrOrder[$i][1] = '';
            $arrOrder[$i][2] = '合計';
            $arrOrder[$i][3] = $cusShipping->getCusTotal();
            ...
            foreach ($Order->getTaxableTotalByTaxRate() as $rate => $total) {
                ...
                $arrOrder[$i][3] = $cusShipping->getCusTotalByTax();
            }
            ...
            $arrOrder[$i][1] = '';
            $arrOrder[$i][2] = '請求金額';
            $arrOrder[$i][3] = $cusShipping->getCusPaymentTotal();
